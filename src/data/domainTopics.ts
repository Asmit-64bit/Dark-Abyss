export interface DomainSlotInfo {
  topic: string;
  tags: string[];
  focusArea: string;
...
export const DOMAIN_SLOT_TOPICS: Record<string, Record<number, DomainSlotInfo>> = {
...
/**
 * Returns domain-tailored topic, tags, and suggested focus area for a specific puzzle slot.
 * Falls back to generic slot definitions if the domain is not recognized.
 */
export function getDomainSlotInfo(domain: string | null | undefined, puzzleId: number): DomainSlotInfo {
  ...
  // Fallback if domain match fails or slot index overflows
  return {
